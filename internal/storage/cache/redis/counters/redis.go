...
func New(nodes []string, isCluster bool, password string, l logger.Logger) storage.CountersCache {
	var c redisConn
	if isCluster {
...
func getCounterName(command, user string) string {
	return fmt.Sprintf("%s:{%s}", command, user)
}
